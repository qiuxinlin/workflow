...
jQuery.fn.pagination = function(maxentries, opts){
	opts = jQuery.extend({
		...
		callbackpagesize : function () {return false;}
	},opts||{});
	...
	return this.each(function() {
		...
		function drawLinks() {
            ...
            /****************** Added ***************/
			/*插入一个下拉框，用户选择后进行跳转*/
			var pagecountLabel = '<span>每页</span>';
            $(pagecountLabel).appendTo(panel);
            // console.log('items_per_page-----' + opts.items_per_page );
            var pagecount = '<select id="pagesezePerPage"><option value="10">10</option><option value="30">30</option><option value="50">50</option></select>&nbsp;';
            $(pagecount).appendTo(panel);
            $('#pagesezePerPage').val(opts.items_per_page);
            $("#pagesezePerPage").bind("change",function(evt){
                // console.log('current-----' + current_page);
                var callbackpagesize = opts.callbackpagesize;
                callbackpagesize($(this).val());
            });
            /****************** Added End ******************/
	        /****************** Added ***************/
